...
class LayerManagerTool {
    constructor(limit = 16, callback_add_layer, callback_checkbox_event, callback_delete_layer, callback_layer_count, callback_onclick_event, callback_slide_event, callback_swap_layers, callback_get_error_parallel_array, callback_get_non_error_background_color) {
        ...
        this.list = new GuiCheckList([1, this.layersLimit], [this.layoutManager.width(), 800 - 280], 20, false, this.callback_swap_layers, (event) => {
...
async function main() {
    ...
    let low_fps = true;
    let draw = false;
    touchListener.registerCallBack("touchstart", (event) => event.touchPos[0] > (game.width - fps_text_width - 10) && event.touchPos[1] < +ctx.font.split('px')[0] * 1.2, (event) => render_fps = !render_fps);
